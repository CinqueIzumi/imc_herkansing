...
void screen_temperature_task_simplified(void *pvParameters)
{
    /* Initialize the counter integer */
    int counter = 0;
    ...
    for(; ;)
    {
        ...
        if(result == pdPASS)
        {
            ...
            /* Convert the counter integer to a char array */
            char snum_current_counter[12];
            sprintf(snum_current_counter, "%d", counter);

            /* Clear the LCD and write the counter value */
            i2c_lcd1602_clear           (lcd_info);
...
/* A function which will update the sensor struct, to keep track of the current values */
void update_sensor_struct(void)
{
...
/* A function which executes when the rotary encoder has been clicked */
void on_rotary_clicked_simplified()
{
...
/* A function which manages switching between the separate screens */
void switch_screen_states(int16_t newState)
{
